...
- **2026-08-06 (later) · Added real Viator product photos (with required attribution) next to every affiliate link inserted earlier this session.** Same combined effort as medellin.guide and thecartagena.guide. Built a master image lookup keyed by Viator product code (640 products sampled network-wide), each with a real hotlinked `media-cdn.tripadvisor.com` CDN URL, since Viator's Partner Program terms require photo credit for used images. Every inserted image is wrapped in `<figure class="viator-pick"><img ...><figcaption>Photo via Viator</figcaption></figure>` (Spanish sections get "Foto vía Viator"), placed next to the paragraph that already links that product, `loading="lazy"`. 20 files got 64 image blocks total. 2 generic-destination links (`best-coworking-spaces-barranquilla`, `san-basilio-de-palenque`, both pointing at destination-wide URLs with no specific product code) correctly skipped, no product-level image exists for a generic link. Verified 0 em dashes introduced (1 pre-existing em dash found in an unrelated file, `whats-happening-in-barranquilla-week-of-may-18-2026`, not touched by this edit). **Not yet committed**, same sandbox git-lock issue. **Deploy, from your machine:** `cd ~/code/barranquilla-guide && rm -f .git/index.lock && git add -A && git commit -m "add real Viator product photos next to affiliate links" && git push`.

- **2026-08-06 · Viator affiliate monetization pass: real, tracked tour-booking links inserted network-wide, this repo's slice.** Same combined effort as medellin.guide and thecartagena.guide, same session. Mike supplied a real Viator Partner API key and confirmed his only configured affiliate pid is `P00296226` (tied to barranquilla.guide's campaign in Viator's portal). Per Viator's own docs, one pid can serve multiple sites via a `campaign` URL parameter, so this same pid is used across all 3 sites with `&campaign=barranquillaguide` for this site's attribution. Used `/partner/destinations` + `/partner/products/search` to resolve Barranquilla's real destination ID (50228, confirmed via the API, not guessed) and pull its live product catalog. Note: Barranquilla is a genuinely small Viator market, 55 total live products vs. 1,300+ for Medellín/Cartagena, so coverage here is real but inherently capped, this isn't under-monetization, it's an accurate reflection of a much smaller tour market. Round 1 (14 files): fixed/added real product links on `barranquilla-bocas-de-cenizas-tour`, `barranquilla-caribbean-night-break-tour`, `barranquilla-food-tour-downtown`, `barranquilla-mallorquin-ecopark-tour`, `barranquilla-nightlife-bars-clubs`, `barranquilla-private-bike-tour`, `barranquilla-walking-tour-el-prado`, `best-bars-craft-drinks-barranquilla`, `best-tours-experiences-barranquilla`, `gran-malecon-carnaval-museum-barranquilla-city-tour`, `neighborhoods/el-prado`, `popular-sports-in-barranquilla`, `puerto-mocho`, `things-to-do-around-barranquilla`. Several of these article slugs already matched a specific real Viator product almost 1:1 (e.g. the bocas-de-cenizas article ↔ the actual Magdalena/Bocas de Ceniza boat tour product), so those got a straight tracking-param fix rather than a new insertion. Round 2 (6 more files, using an expanded 55-product table plus a 50-product Santa Marta table since several articles mention Santa Marta/Tayrona as a nearby day trip): `barranquilla-airport-guide` (real private-transfer + BAQ→Santa Marta shuttle products), `day-trips-from-barranquilla` (added a private Santa Marta transfer and a Tayrona/Cabo San Juan hike tour to a section that only had a hotel-booking link before), `fun-things-to-do-in-barranquilla-as-a-group`, `barranquilla-carnival-complete-guide`, `your-first-week-in-barranquilla-the-essential-arrival-checklist-2026`, `everything-you-need-to-know-about-barranquilla` (added the Gabriel García Márquez literary tour, a real dedicated product). Site total: 22 files now carry a Viator link (was 0 before this session). Deliberately concluded round 2 there rather than forcing weak matches into banking/tax/visa/health-insurance guides or into `moving-to-barranquilla`/`getting-around-barranquilla`/`barranquilla-neighborhoods`, none had a genuine bookable-product match. Verified: balanced tags, 0 em dashes, every URL copied verbatim from the API response, `rel="noopener sponsored" target="_blank"` matching the site's existing affiliate-link convention. **Not yet committed**, this sandbox's git lock file (`.git/index.lock`) can't be removed this session (same recurring issue as prior sessions). **Deploy, from your own machine:** `cd ~/code/barranquilla-guide && rm -f .git/index.lock && git add -A && git status` (review, this touches 20 content files plus this CLAUDE.md entry), then `git commit -m "add tracked Viator affiliate links across matching articles" && git push`.

- **2026-07-26 (later x7) · Synced `asset-protection-succession-panama` content to match the now-repaired medellin.guide version verbatim (translated headings/wording, same structure), after medellin's copy got corrupted and manually rebuilt by a bad dashboard-editor save earlier today.** Barranquilla's copy was never corrupted (only medellin's was), but per Mike's explicit request the two sister-site copies should read exactly the same going forward: replaced the body prose (intro paragraphs, all 7 `<h2>` sections, the mistakes list, FAQ, and related-reading line) with the medellin wording, keeping this site's own internal link targets (`/tax-residency-colombia-2026-guide/` for the 183-day FAQ answer) and its own nav/footer/CSS/domain untouched. Also updated the mid-article Catalina callout to the newer "she'll loop us in to make the right introduction" wording. Bumped `article:modified_time` + JSON-LD `dateModified` to 2026-07-26. Verified: div/section/article/ul/li/p/h2/h3/aside tags balanced, 0 em dashes. **Deploy:** `cd ~/code/barranquilla-guide && git add asset-protection-succession-panama CLAUDE.md && git commit -m "sync asset-protection-succession-panama content with medellin.guide" && git push`.

- **2026-07-26 (later x6) · Reworded the mid-article referral callout on `second-residency-plan-b-panama-colombia`, network-wide.** Same fix as the same-day medellin-guide entry, per Mike's replacement text (corrected once to bold+link only "Catalina," drop the comma after it): "Tell our AI receptionist, **Catalina** your situation and she'll recommend local attorneys and accountants we trust." **Deploy:** `cd ~/code/barranquilla-guide && git add second-residency-plan-b-panama-colombia CLAUDE.md && git commit -m "reword AI-receptionist referral callout" && git push`.

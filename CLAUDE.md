...
There's no dedicated "partner recommendation" callout component on this site yet (Phase 3 of the content strategy, not built, partner data in `partnerships-db` is currently empty for Barranquilla, zero rows), don't invent one ahead of time.
...
- **2026-06-16** · Shared featured images + in-post concierge hints. (1) **Shared syndicated images:** the ~11 Colombia-wide articles republished on each guide (canadian-taxes, travel-insurance, citizenship-exam, como-hablar-con-gringos, Amazon, maid-salary, estrato, voting, bringing-money, health-insurance, tax-residency) now use ONE identical real photo across all three guides, copied into /img/shared/<topic> and pointed at by the hero, og:image (was the dark title card) and section card. (2) **High-intent in-post hint:** the mid-article cat-cta-mid callout on Moving Here + Medical + weddings posts now reads as a direct concierge offer ("Rather not do this part yourself? ... Mike and Santi do it every week", medical and wedding variants too), bilingual EN/ES, instead of the generic "ask Catalina" prompt. Strategy: push the service through the posts themselves, not a standalone section. 0 em dashes. **Deploy:** git add -A && git commit && git push.

- **2026-06-15 (later x1)** · Header/footer consistency pass (Option A, matching cartagena + medellin). Added the Search-with-Catalina form to all content pages; fixed content-page brand logos (were using the post hero, now `/logo-b.webp`); stamped one canonical pb-top + header + drawer + footer onto every page (byte-identical across all 124 pages); added the PymeWebPro credit bar under the footer. QA: 0 em dashes. **Deploy:** `cd ~/code/barranquilla-guide && rm -f .git/*.lock && git add -A && git commit -m "Unify header/footer + site-wide search bar + logo fix + Pyme bar" && git push`.

- **2026-06-15** · Ported the unified **6-category structure** (shared verbatim with medellin + cartagena) onto barranquilla.guide. The 6: **Things to Do** (`/section/explore/`) · **Just Visiting** (`/section/just-visiting/`, NEW) · **Moving Here** (`/section/moving-here/`, NEW) · **Food & Drink** (`/section/eat-drink/`) · **Medical** (`/section/health/`) · **Life & Culture** (`/section/daily-life/`). Getting Around → Just Visiting; News/Now stays a homepage block + newsletter feed (drawer + footer); Neighborhoods → Life & Culture; Map → footer only. New nav/drawer/footer on all 124 pages. Rebuilt all 6 section pages from the same bilingual template used on cartagena (re-skinned: logo-b, GA `G-FKFW8ZQJPN`, css `?v=20260529a`, catalina.barranquilla.guide, barranquilla cornerstone + sister links), all 85 posts sorted in. **Card images:** real photos only · 69 cards use the post's curated photo or `/img/<slug>-place-*` topical shot, 12 utility cards (taxes/citizenship/shipping/voting/sim/scams/working-remotely/movie-theaters/popular-sports/everything-to-know/first-week/como-hablar) fall back to `/img/barranquilla-city-panoramic-view.jpg` (real cityscape, NOT a title card · candidates for a future generated `/img/1200/<slug>-hero.webp`). 0 `/img/heroes/` title cards in the 6 pages. Homepage `#browse` grid expanded 4 → 6 cards (real photos). Retired sections → stubs + `_redirects` 301s; internal links remapped. Added `/*.png` etc. to `.gitignore`. QA: 0 em dashes, all card links + nav targets resolve, no Cartagena leftovers (CTA + cornerstone rebranded). **Deploy:** `cd ~/code/barranquilla-guide && rm -f .git/*.lock && git add -A && git commit -m "Restructure nav into 6 categories + rebuild section pages + homepage cards" && git push`.

- **2026-06-04** · Ported the Cartagena 7-section IA rebuild to barranquilla.guide. New nav across all 123 pages + 404.html (header, mobile drawer, footer Sections list, homepage, plus the entity-encoded and plain-label variant pages): Things to Do · Eat & Drink · Sleep · Day to Day · Health & Medical · Culture · Local news · City Map (City Map kept per Mike, unlike Cartagena). Retired `Live` and `How-to Guides` as sections. Built 3 new section landing pages from the eat-drink template: `/section/daily-life/` (35 cards, the practical Live + How-to Guides articles), `/section/health/` (9, the medical cluster · 6 of them had no card image so they reuse `healthcare-barranquilla-600x400.jpg`), `/section/culture/` (5: Carnaval, salsa/cumbia dancing, San Basilio de Palenque, weddings, + the Cultura-eyebrow piece). Moved Carnaval/dancing/Palenque out of Explore into Culture (Explore 21 → 18). Classification was driven by each article's existing eyebrow label (reliable) plus health/culture override lists, NOT the section-grid harvest (which was contaminated by mixed card formats). Reassigned 46 of 81 article pages' breadcrumbs (header eyebrow + visible breadcrumb link + JSON-LD + related-card labels + the linked-label `<span class="label"><a href=...>` variant) to their new section. Homepage: the "Live" feature card became "Day to Day", the "All how-to guides" CTA now points at `/section/daily-life/` ("All everyday guides"). `sitemap.xml`: removed live + guides, added daily-life + health + culture. `_redirects`: `/section/live/* → /section/daily-life/:splat 301`, `/section/guides/* → / 301` (and dropped the now-dead `/section/{live,guides}/page/*` rules). Old `/section/live/` + `/section/guides/` retired from the deploy (git rm --cached). QA: 0 em/en dashes across all changed + new HTML, single h1 per new section page, 0 broken card links, span balance preserved, all 8 nav destinations resolve. Scripts kept in `outputs/`: `baq_nav_swap.py`, `baq_build_sections2.py`, `baq_breadcrumbs.py`. Known minor: a handful of related-card eyebrows in non-article-card formats may still read old labels; harmless. **Deploy:** committed + pushed to main (Pages auto-deploys).

- **2026-05-29 (later x3)** · Removed the "Chat with Catalina / Habla con Catalina" link from the top navigation bar (`.pb-l` div) on every HTML page. Script scanned each page, found the bare `<a href="#" onclick="var l=document.querySelector('.cat-launcher')...">` link inside `<div class="pb-l">` (no class attribute, no `data-pb-close`), and stripped the entire `<a>` tag. Drawer (mobile), footer, and hero CTA versions of the same link kept their wording. Floating launcher bubble untouched · chat still works from any page. Net: 124 BAQ pages stripped (242 across all 3 guide sites). **Deploy:** `cd ~/code/barranquilla-guide && git add -A && git commit -m "Remove 'Chat with Catalina' link from top nav (keep launcher + drawer + footer)" && git push`.

- **2026-05-29 (later x2)** · Fixed the same `.magazine-grid` bug Mike caught on cartagena. The "Related articles" block at the bottom of every post uses `<div class="magazine-grid" style="grid-template-columns: repeat(3, 1fr);">` but `.magazine-grid` was never defined in `css/site.css`, so without `display: grid` the inline template was a no-op, each `.article-card` stretched full container width, and any portrait-aspect related image rendered ~1200×900 via the 4:3 `aspect-ratio` on `.article-card-image`. Added `.magazine-section`, `.magazine-header`, and `.magazine-grid` rules to `css/site.css` (1 col mobile → 2 cols ≥700px → 3 cols ≥1000px). Bumped the `site.css?v=` cache-buster from `20260528b` to `20260529a` site-wide via `sed`. Fixes 68 post pages. Files: `css/site.css`, every `*.html` with a stylesheet link. **Deploy:** `cd ~/code/barranquilla-guide && git add -A && git commit -m "Fix magazine-grid related-articles layout (giant image bug)" && git push`.

- **2026-05-29 (later x1)** · Phase 4 hero-card generation for the 11 new Phase 6 BAQ pages. Same script as the CTG companion (`outputs/gen_hero_cards.py`, Pillow-based, matches the 2026-05-28 Phase 4 dark-navy spec: 1200×630 JPG, navy #1f1a14 background, gold #c9a84c left accent bar, gold ring medallion top-left with "B" brand letter, gold uppercase category label, cream wrapped Liberation Serif Bold title, "barranquilla.guide" wordmark bottom-left with gold dot, right-panel gold motif line). Generated 11 hero JPGs into `img/heroes/`: voting-us-elections-from-colombia, canadian-taxes-colombia-seniors, como-hablar-con-gringos, renting-a-car-barranquilla, vegetarian-vegan-celiac-food-barranquilla, health-insurance-colombia, best-hospitals-barranquilla, best-dermatologists-barranquilla, best-hair-transplant-clinics-barranquilla, best-ivf-fertility-clinics-barranquilla, best-bariatric-surgery-clinics-barranquilla. Each ~27-34 KB. Fixes og:image 404s on social-share previews. **Deploy:** `cd ~/code/barranquilla-guide && git add -A && git commit -m "Phase 4 follow-up: hero cards for 11 new Phase 6 pages" && git push`.

- **2026-05-29** · Phase 6 Bucket B (medical tourism), BAQ batch + national health-insurance article. Shipped 6 verified-clinic-only articles, all based on the source-cited research brief in `~/code/_shared/phase-6-medical-research-brief.md` (no fabricated clinic names). New pages: `health-insurance-colombia/` (national content, identical body to the CTG version, covers 2024-2026 EPS crisis with Sanitas+Nueva intervened/Coomeva liquidated/Salud Total most stable, prepagada providers, international insurance options, the visa-insurance-EPS-mismatch gotcha, 3-stack decision framework), `best-hospitals-barranquilla/` (honest framing: no JCI in BAQ; Clínica Portoazul Auna is the international-patient leader, Newsweek 2024 listed; LMCI with CIOM oncology, Iberoamérica/Keralty group, CGN largest installed capacity, Bonnadona oncology+cardio; CARI public + Hospital Niño Jesús; decision table), `best-dermatologists-barranquilla/` (Dr. Iván Diazgranados + Dra. Magaly Domínguez, Asocolderma + Dermocaribe credential check, coastal-skin context, unbranded-injectable warning), `best-hair-transplant-clinics-barranquilla/` (Mediarte HQ chain, Capilclinic, Kaloni, Dermaster Hair Medical; FUE vs FUT, per-graft pricing COP 6-9k, recovery in BAQ heat), `best-ivf-fertility-clinics-barranquilla/` (headline FertilityCare Colombia with ICMART+ASRM+RedLARA credentials, Procrear/Esperanza de Vida, InSer national chain, Profamilia baseline; IUI to PGT-A pricing; Colombian fertility law notes including same-sex couples + surrogacy gray zone), `best-bariatric-surgery-clinics-barranquilla/` (headline Dr. Jorge Daes Daccarett: 10,000+ laparoscopic cases, 4,000+ sleeve/bypass, president-elect American Hernia Society, operates at Portoazul+Iberoamérica; Dr. Ricardo Daguer Díaz BAQ+CTG, Dr. Santiago Gómez Correa; sleeve/bypass/balloon/revision; eligibility criteria; post-op body contouring cross-link). Each is full BAQ chrome (pb-* nav with City Map slot, GA4 G-FKFW8ZQJPN, Catalina widget at catalina.barranquilla.guide, self-hosted Inter+Playfair preloads + Fraunces from Google, newsletter form, "Costeño-checked" banner, BAQ footer "Live in Barranquilla" section). Internal links use existing BAQ slugs. No em dashes. Hero images referenced at `/img/heroes/<slug>.jpg` not yet generated (Phase 4 hero-card script re-run pending). Updated `sitemap.xml` with 6 new entries (lastmod 2026-05-29, priority 0.8). The honest-framing rule held strictly: no JCI in BAQ (state it), Dr. Daes is the headline credentialed bariatric name on the coast, FertilityCare is the credential leader for IVF, BAQ is stronger than CTG for hair transplant. **Deploy:** `cd ~/code/barranquilla-guide && git add -A && git commit -m "Phase 6 Bucket B: 5 BAQ medical-tourism articles + national health-insurance guide" && git push`. **Follow-up:** re-run hero-card script to generate the 6 missing hero JPGs.

- **2026-05-28 (later x6)** · Phase 6 content parity, BAQ batch of MDE→BAQ ports. Added 5 new national-content articles ported from `medellin-guide/guides/` and re-localized for Barranquilla: `voting-us-elections-from-colombia/` (highlights the U.S. Consular Agency in El Prado, the only one on the coast), `canadian-taxes-colombia-seniors/` (notes the Honorary Consul of Canada in BAQ, El Prado/Villa Country residential context, DHL Las Américas courier), `como-hablar-con-gringos/` (Spanish-language, BAQ-specific gringo personas including el ejecutivo del sector industrial, misionero evangélico, visitante de Carnaval, fan de Junior, plus arroyo warning), `renting-a-car-barranquilla/` (BAQ is the most car-friendly of the three Caribbean cities; emphasizes the arroyo death-trap warning, Santa Marta/Tayrona/Mompox/Tubará/Ruta del Girasol day-trip math, Carnaval parade route closures), and `vegetarian-vegan-celiac-food-barranquilla/` (honest framing that BAQ's veg scene is the thinnest of the three; leans on the Lebanese/Italian community presence, Govinda's, supermarket self-catering at Carulla/Olímpica/Jumbo, Granabastos market). Each is full BAQ chrome (pb-* nav with City Map instead of Sleep, Catalina widget at `catalina.barranquilla.guide`, GA4 G-FKFW8ZQJPN, self-hosted Inter+Playfair preloads + Fraunces from Google, newsletter form to `colguides.com/subscribe`, BAQ footer with "Live in Barranquilla" section, "Costeño-checked" tagline). All internal links use existing BAQ slugs. No em dashes. Heroes referenced at `/img/heroes/<slug>.jpg` but not yet generated, re-run `outputs/gen_hero_cards.py` to fill them in a follow-up commit. Updated `sitemap.xml` with 5 new entries (lastmod 2026-05-28). Companion CTG batch was shipped same day. Source of overall parity audit: `~/code/_shared/phase-6-content-parity-audit.md`. **Deploy:** `cd ~/code/barranquilla-guide && git add -A && git commit -m "Phase 6 batch: port 5 national-content guides from MDE to BAQ" && git push`.

- **2026-05-27 (later x5)** · AI-search visibility phase 1: new `/llms.txt` at site root + significantly expanded homepage schema. `llms.txt` (~75 lines, llmstxt.org spec) describes the site (Carnival, neighborhoods, healthcare, food, tours, day trips, family travel, comparisons to Medellín and Bogotá), what it does not cover (no tourism-board affiliation, no real-estate listings, no medical/legal authority), coverage in COP, key URLs, founders, honest framing (small team + AI, not a Barranquilla office), and a citation policy telling AI assistants to link back. On `index.html`, the existing Organization block was augmented with `knowsLanguage` and a `knowsAbout` list of 23 topical authorities (neighborhoods, Carnaval, Gran Malecón, Bocas de Cenizas, Mallorquín Ecopark, cumbia, champeta, Caribbean cuisine, cost of living, healthcare, dental tourism, etc.). Added a brand-new LocalBusiness JSON-LD block with `areaServed` covering Barranquilla + Puerto Colombia + Soledad + Tubará + Atlántico. Added a brand-new FAQPage JSON-LD block with 7 high-intent Q+A pairs covering safety, Carnaval dates and worth-it framing, cost of living vs Medellín, best expat neighborhoods, Bocas de Cenizas mechanics, dental tourism, visa basics. No content changes to the visible page; pure schema enrichment. All 5 JSON-LD blocks parse cleanly. Em-dash 0 in both files. Companion sweep on `medellin.guide` and `thecartagena.guide` same day. **Deploy:** `git add -A && git commit -m "AI-search phase 1: llms.txt + LocalBusiness + FAQPage + Organization knowsAbout" && git push`.

- **2026-05-28 (later x4)** · Phase 5: added Catalina bottom CTA (`cat-cta-bot` section) to all 7 `/section/*/index.html` archive pages and `category/now/index.html` (8 pages total). Anchor: injected before `<!-- newsletter-prefoot:v1 -->`. Sentinel: `<!-- catalina-section-cta v1 -->` (idempotent). No article-body wrapping on these pages so top/mid CTAs are not applicable. Text is bilingual `pb-en`/`pb-es`. No em dashes. Script: `outputs/inject_section_ctas.py`. Files: `section/*/index.html` (7), `category/now/index.html`. Deploy: `git add -A && git commit -m "Phase 5: add Catalina CTAs to section archive pages" && git push`.

- **2026-05-28 (later x3)** · Phase 4: generated 98 dark navy 1200x630 hero cards for all article pages. Design mirrors medellin.guide: navy (#1f1a14) background, gold left accent bar, logo-b.png medallion top-left, gold category label, Liberation Serif Bold title (cream, wrapped), subtle Caribbean wave motif right panel, "barranquilla.guide" wordmark bottom-left. Updated `og:image`, `og:image:secure_url`, and in-page `article-hero-image` img src on all 98 pages. Cards saved to `img/heroes/<slug>.jpg` (~38-44KB each). Script: `outputs/gen_hero_cards.py`. Files: `img/heroes/*.jpg` (98 new), 98 `*/index.html`. Deploy: `git add -A && git commit -m "Phase 4: add branded dark hero cards to all article pages" && git push`.

- **2026-05-28 (later x2)** · Phase 3b: injected bilingual Catalina CTAs (top/mid/bottom) into all 100 article pages. Top CTA uses `.cat-cta-inline` as first child of `.article-body`; mid CTA uses `.cat-cta-mid` after the middle H2; bottom CTA uses `.cat-cta-bot` before `</div><!-- .article-body -->`. All CTAs have `pb-en`/`pb-es` bilingual spans. `san-basilio-de-palenque` handled as a special case (split `data-lang` article bodies). `terms` page correctly skipped (legal page). QA: 100/100 article pages have CTAs. Script: `outputs/inject_catalina_ctas.py`. Files: 100 `*/index.html`. Deploy: `git add -A && git commit -m "Phase 3b: inject bilingual Catalina CTAs into all article pages" && git push`.

- **2026-05-28 (later)** · Canonical site.css Phase 3: added `.cat-cta-mid` (dark navy aside) and `.cat-cta-bot` (pale-gold card) Catalina CTA classes to `css/site.css` (canonical, identical to medellin + cartagena). Bumped cache-bust version `?v=20260528a → ?v=20260528b` across all 112 HTML files. No HTML content changes on barranquilla pages this phase. Files: `css/site.css`, all `*.html`. Deploy: `git add -A && git commit -m "CSS: add cat-cta-mid + cat-cta-bot classes, bump version to 20260528b" && git push`.

- **2026-05-28** · Canonical site.css Phase 1: aligned with medellin.guide visual style + added shared component classes. (1) Fixed `.article-body` `line-height: 1.75 → 1.62` to match medellin.guide's tighter reading rhythm. (2) Added `.article-hero img` rule (rounded 12px corners + shadow) so both barranquilla-style `.article-hero-image` wrappers and medellin-style `figure.article-hero > img` render correctly from the same file. (3) Added `.cat-cta-inline` class for the Catalina in-article prompt (gold left-bar card + dark button). (4) Added medellin-compat content classes that appear in ported article HTML: `.breadcrumb`, `.category-tag`, `.article-meta`, `.article-excerpt`, `.neighborhood-card`, `.neighborhood-highlights`, `.highlight-item`, `.best-for`, `.cta-section`, `.cta-buttons`, `.btn-primary`, `.btn-secondary`, `.related-posts`, `.related-grid`, `.related-card-category`, `.guide-toc` — all using `--pb-*` tokens. (5) Bumped cache-bust version `?v=20260527a → ?v=20260528a` across all 112 HTML files. The canonical file at `css/site.css` is now identical across barranquilla, cartagena, and medellin. Files: `css/site.css`, all `*.html`. Deploy: `git add -A && git commit -m "CSS: canonical site.css Phase 1 — line-height, article-hero, Catalina CTA, compat classes" && git push`.

- **2026-05-27 (later x5)** · Palette alignment with medellin.guide. (1) Updated `theme-color` meta from `#1B2A47` to `#1a1a2e` on all 111 article pages (homepage was already correct at `#f6efe2`). (2) Added `.say-box` styles to `css/site.css` using CSS variables (`--pb-paper`, `--pb-bg2`, `--pb-gold`, `--pb-gold-deep`, `--pb-ink`, `--pb-ink-soft`) replacing hardcoded old-palette hex values. (3) Removed inline `<style>` say-box blocks from 33 pages (the 44 pages that had them; 4 had mixed blocks, stripped the say-box rules from those individually). (4) Replaced off-brand purple/pink Viator affiliate CTA gradient (`#2d3561`/`#c05c7e`) with brand `var(--pb-ink)` background + `var(--pb-gold)` button on 6 pages (`things-to-do-around-barranquilla`, `san-basilio-de-palenque`, `best-tours-experiences-barranquilla`, `day-trips-from-barranquilla`, `fun-things-to-do-in-barranquilla-as-a-group`, `barranquilla-food-tour-downtown`). (5) Fixed `--navy` CSS variable fallback to `--pb-ink` in `colombian-citizenship-exam-study-guide`. (6) Bumped site.css cache version `?v=20260516e → ?v=20260527a`. Files: `css/site.css`, all `*.html`. Deploy: `rm .git/index.lock && git add -A && git commit -m "Style: palette alignment with medellin.guide" && git push`.
